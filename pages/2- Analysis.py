...
for col in cols:
    new_df[col] = pd.to_numeric(new_df[col], errors='coerce')  # Convert invalid values to NaN

# Drop rows where any numeric value is NaN
new_df = new_df.dropna(subset=cols)

# Group by 'sector' and calculate mean 
group_df = new_df.groupby('sector').mean(numeric_only=True)[cols]
